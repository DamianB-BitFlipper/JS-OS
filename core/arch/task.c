 ...
 *  *   //saving the old processes register values
 ...
 *  * 
 *  *   if(strlen(task_Name) > sizeof(new_task->name) - 1)
 *  *   {
 *  *     memcpy(new_task->name, task_Name, sizeof(new_task->name) - 1);
 *  *     *(new_task->name + sizeof(new_task->name) - 1) = 0; //terminationg \000
 *  *   }else
 ...
 *  *   new_task->averaged_priority = priority + burst_time;
 *  *   new_task->time_to_run = 0;
 *  *   new_task->time_running = 0;
 *  *   new_task->ready_to_run = TRUE;
 *  *   new_task->page_directory = 0;
 *  * 
 *  *   new_task->next = 0;
 *  * 
 *  *   preempt_task(new_task);
 *  * 
 ...
 *     return;
 * 
 *   //there is only one task, why switch to it?, just exit
 *   if(!ready_queue->next)
 *     return;
 * 
 *   // Read esp, ebp now for saving later on.
...
void __test__()
{
  ...
  u32int pid = fork(PRIO_LOW, PROC_MED, "wopw");
